...
def update_attendance_list_per_training_per_user(current_user, training_id):
    from main import db
    from api.training import get_attendance_list_by_training
    ...
    try:
        ...
        db.session.commit()
        return jsonify({"success": True, "message": "update attendance for training: " + training_id + " for user: " + str(user_id) + " successfully" })
    except:
