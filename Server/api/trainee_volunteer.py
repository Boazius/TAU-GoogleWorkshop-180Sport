...
def delete_message(current_user, user_id, training_id):
    from main import db
    if current_user.user_type != 1 and current_user.id != user_id:
        return jsonify({"success": False, "message": "User cannot update message, unless it is the fit user"}), 401
    training_from_db = db.session.query(Training).filter_by(id=training_id).first()
    if not training_from_db:
        return jsonify({'success': False, 'message': 'No training found!'})
    notes = json.loads(training_from_db.notes)
...
def get_closest_training(current_user, user_id):
    ...
    if user_groups == "" or user_groups is None:
...
def get_closest_training_by_training_id(current_user, user_id, training_id):
    ...
    if training_from_db.group_id not in current_user.group_ids:
        return jsonify({"success": False, "message": "User cannot get training, unless the user is in the fit group"}), 401
    return jsonify({"success": True, "message": training_from_db.to_dict()})
...
def update_attendance(current_user, user_id):
    ...
    try:
        ...
        if attendance is None:
            return jsonify({"success": False, "message": "attendance list is empty"})
        if str(user_id) not in attendance.keys():
            return jsonify({"success": False, "message": "user not in attendance training"})
